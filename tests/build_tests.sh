...
RUN ln -s /usr/bin/python2 /usr/bin/python && \
    ...
    python2 -c "import site; print('site-packages:', site.getsitepackages())" || true

# Clone repositories
RUN mkdir -p /root/bin && \
    cd /root/bin && \
    git clone --depth=1 https://chromium.googlesource.com/chromium/tools/depot_tools || git clone --depth=1 https://chromium.googlesource.com/chromium/tools/depot_tools && \
    export PATH="$PATH:/root/bin/depot_tools" && \
    ls -l /root/bin/depot_tools/gyp && \
    /root/bin/depot_tools/gyp --version || echo "gyp version failed" && \
    git clone -b latest-stable --depth=1 https://github.com/apache/incubator-pagespeed-mod /tmp/mod_pagespeed || git clone -b latest-stable --depth=1 https://github.com/apache/incubator-pagespeed-mod /tmp/mod_pagespeed && \
    cd /tmp/mod_pagespeed && \
    git submodule update --init --depth=1 third_party/apr third_party/aprutil third_party/httpd24 third_party/libpng third_party/zlib && \
    ls -l /tmp/mod_pagespeed/build/mod_pagespeed.gyp

# Build mod_pagespeed
RUN cd /tmp/mod_pagespeed && \
    export PATH="$PATH:/root/bin/depot_tools" && \
    if grep -q "build/gyp_chromium" /Dockerfile; then echo "ERROR: Outdated Dockerfile detected! Use depot_tools/gyp instead." && exit 1; fi && \
    /root/bin/depot_tools/gyp --depth=. build/mod_pagespeed.gyp -D use_system_libs=1 && \
