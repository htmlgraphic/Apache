...
def bootstrap():
    ...
    apt("tasksel rsync")
    apt("mysql-server libapache2-mod-auth-mysql php5-mysql")
    ...
    sudo('''sh -c "echo 'ServerName db.htmlgraphic.com' > /etc/apache2/conf-available/servername.conf"''')
...
def start_server():
    ...
    try:
        sudo("service mysql start", pty=False)
    except:
        pass
...
def stop_server():
    ...
    try:
        sudo("service mysql stop", pty=False)
    except:
        pass
...
def restart_server():
    ...
    sudo("server mysql restart", pty=False) #running this command after the system is up causes an issue, since the "service apache2 start" command does not work in this script, can we check if apache is running and skip
